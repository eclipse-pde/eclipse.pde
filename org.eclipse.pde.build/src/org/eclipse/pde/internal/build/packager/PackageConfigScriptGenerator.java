...
public class PackageConfigScriptGenerator extends AssembleConfigScriptGenerator {
	...
	protected void generateGatherBinPartsCalls() { //TODO Here we should try to use cp because otherwise we will loose the permissions
		...
		for (int i = 0; i < plugins.length; i++) {
			...
			if (isFolder) {
				script.printCopyTask(null, getPropertyFormat(PROPERTY_ASSEMBLY_TMP) + '/' + getPropertyFormat(PROPERTY_PLUGIN_ARCHIVE_PREFIX) + '/' + ModelBuildScriptGenerator.getNormalizedName(plugins[i]), new FileSet[] {new FileSet(pluginLocation.toOSString(), null, null, null, excludedFiles, null, null)}, false);
			} else {
		...
		for (int i = 0; i < features.length; i++) {
			...
			script.printCopyTask(null, getPropertyFormat(PROPERTY_ASSEMBLY_TMP) + '/' + getPropertyFormat(PROPERTY_FEATURE_ARCHIVE_PREFIX) + '/' + FeatureBuildScriptGenerator.getNormalizedName(features[i]), new FileSet[] {new FileSet(featureLocation.toOSString(), null, null, null, null, null, null)}, false);
		}
		...
		if (rootFileProviders.size() != 0) {
			...
			script.printCopyTask(null, target, new FileSet[] { rootFiles }, false);
		}
	}
