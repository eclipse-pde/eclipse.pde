...
public void addClassFolder(IPath projectPath, IPath classFolderPath, boolean isExported) throws JavaModelException {
	...
	public IPath addProject(String projectName, String compliance){
		...
		if ("1.5".equals(compliance)) {
			requiredComplianceFlag = AbstractBuilderTest.F_1_5;
			compilerVersion = CompilerOptions.VERSION_1_5;
		}
		else if ("1.6".equals(compliance)) {
			requiredComplianceFlag = AbstractBuilderTest.F_1_6;
			compilerVersion = CompilerOptions.VERSION_1_6;
		}
		else if ("1.7".equals(compliance)) {
			requiredComplianceFlag = AbstractBuilderTest.F_1_7;
		...
		if (requiredComplianceFlag != 0) {
			if ((AbstractBuilderTest.getPossibleComplianceLevels() & requiredComplianceFlag) == 0)
...
//			return packageFragmentRootsPath;
	...
	*/
	...
	/**
	 * Return all markers with workspace.
	 */
	 ...
	 */
	public IMarker[] getMarkersFor(IPath path){
		return getMarkersFor(path, null);
	}
	/**
	 * Return all problems with the specified element.
	 */
	public IMarker[] getMarkersFor(IPath path, String additionalMarkerType){
		...
		try {
			...
			IMarker[] markers = resource.findMarkers(IApiMarkerConstants.API_USAGE_PROBLEM_MARKER, true, IResource.DEPTH_INFINITE);
			for (int i = 0; i < markers.length; i++) {
				problems.add(markers[i]);
			}

			markers = resource.findMarkers(IApiMarkerConstants.BINARY_COMPATIBILITY_PROBLEM_MARKER, true, IResource.DEPTH_INFINITE);
			for (int i = 0; i < markers.length; i++) {
				problems.add(markers[i]);
			}
			
			markers = resource.findMarkers(IApiMarkerConstants.DEFAULT_API_PROFILE_PROBLEM_MARKER, true, IResource.DEPTH_INFINITE);
			for (int i = 0; i < markers.length; i++) {
				problems.add(markers[i]);
			}
			
			markers = resource.findMarkers(IApiMarkerConstants.SINCE_TAGS_PROBLEM_MARKER, true, IResource.DEPTH_INFINITE);
			for (int i = 0; i < markers.length; i++) {
				problems.add(markers[i]);
			}

			markers = resource.findMarkers(IApiMarkerConstants.VERSION_NUMBERING_PROBLEM_MARKER, true, IResource.DEPTH_INFINITE);
			for (int i = 0; i < markers.length; i++) {
				problems.add(markers[i]);
			}
			...
			IMarker[] result = new IMarker[problems.size()];
