...
public class InterfaceCompatibilityRestrictionTests extends InterfaceCompatibilityTests {
	 ...
	 */
	protected static IPath WORKSPACE_CLASSES_PACKAGE_A = IPath.fromOSString("bundle.a/src/a/interfaces/restrictions"); //$NON-NLS-1$
	...
	private void xAddNoImplement(boolean incremental) throws Exception {
		...
		args[0] = new String[] { "@noimplement", PACKAGE_PREFIX + "AddNoImplement" }; //$NON-NLS-1$ //$NON-NLS-2$
	...
	private void xAddNoExtendNoImplement(boolean inc) {
		try {
			...
			setExpectedProblemIds(ids);
			String[][] args = new String[1][];
			args[0] = new String[] { "@noimplement", PACKAGE_PREFIX + "AddNoExtendNoImplement" }; //$NON-NLS-1$ //$NON-NLS-2$
			setExpectedMessageArgs(args);
			performCompatibilityTest(filePath, inc);
		}
		catch(Exception e) {
