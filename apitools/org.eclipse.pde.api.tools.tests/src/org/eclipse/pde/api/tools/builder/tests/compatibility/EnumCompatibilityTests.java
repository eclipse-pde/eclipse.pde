...
public class EnumCompatibilityTests extends CompatibilityTest {
	 ...
	 */
	protected static IPath WORKSPACE_CLASSES_PACKAGE_A = IPath.fromOSString("bundle.a/src/a/enums"); //$NON-NLS-1$
	...
	private void xRemoveSuperInterface(boolean incremental) throws Exception {
		...
		setExpectedProblemIds(ids);
		String[][] args = new String[1][];
		args[0] = new String[] { PACKAGE_PREFIX + "RemoveSuperInterface", "a.classes.hierarchy.InterfaceA" }; //$NON-NLS-1$ //$NON-NLS-2$
		setExpectedMessageArgs(args);
		performCompatibilityTest(filePath, incremental);
	}
