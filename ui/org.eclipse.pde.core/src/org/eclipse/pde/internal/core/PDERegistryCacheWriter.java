...
public class PDERegistryCacheWriter {
	// See PDERegistryCacheReader for constants commonly used here too.
	...
	public void writeConfigurationElement(
		ConfigurationElementModel configElement,
		...
		try {
			...
			if (configElementIndex != -1) {
				...
				writeLabel(
					PDERegistryCacheReader.CONFIGURATION_ELEMENT_INDEX_LABEL,
					out);
			...
			ConfigurationElementModel[] subElements =
				configElement.getSubElements();
			if (subElements != null) {
				writeLabel(
					PDERegistryCacheReader.SUBELEMENTS_LENGTH_LABEL,
					out);
			...
			writeLabel(
				PDERegistryCacheReader.CONFIGURATION_ELEMENT_PARENT_LABEL,
				out);
			...
			writeLabel(
				PDERegistryCacheReader.CONFIGURATION_ELEMENT_END_LABEL,
				out);
		} catch (IOException ioe) {
		}
	}
	
	public void writeConfigurationProperty(
		ConfigurationPropertyModel configProperty,
		DataOutputStream out) {
		try {
			String outString;

			writeLabel(
				PDERegistryCacheReader.CONFIGURATION_PROPERTY_LABEL,
				out);
			...
			writeLabel(
				PDERegistryCacheReader.CONFIGURATION_PROPERTY_END_LABEL,
				out);
		} catch (IOException ioe) {
		}
	}
	...
	public void writeExtension(
		...
		try {
			...
			if ((outString = extension.getExtensionPoint()) != null) {
				writeLabel(
					PDERegistryCacheReader.EXTENSION_EXT_POINT_NAME_LABEL,
					out);
				out.writeUTF(outString);
			}
			...
			if (parentIndex != -1) {
				...
				if (parent instanceof PluginDescriptorModel) {
					...
					writeLabel(
						PDERegistryCacheReader.FRAGMENT_INDEX_LABEL,
						out);
				}
	...
	public void writeExtensionPoint(
		ExtensionPointModel extPoint,
		DataOutputStream out) {
		try {
			...
			if (extensionPointIndex != -1) {
				...
				writeLabel(
					PDERegistryCacheReader.EXTENSION_POINT_INDEX_LABEL,
					out);
			...
			writeLabel(
				PDERegistryCacheReader.PLUGIN_EXTENSION_POINT_LABEL,
				out);
			...
			if ((outString = extPoint.getSchema()) != null) {
				writeLabel(
					PDERegistryCacheReader.EXTENSION_POINT_SCHEMA_LABEL,
					out);
				out.writeUTF(outString);
			}
			...
			if (parent != null) {
				...
				if (parentIndex != -1) {
					writeLabel(
						PDERegistryCacheReader.EXTENSION_POINT_PARENT_LABEL,
						out);
					out.writeInt(parentIndex);
				}
			}
			...
			if (extLength != 0) {
				writeLabel(
					PDERegistryCacheReader
						.EXTENSION_POINT_EXTENSIONS_LENGTH_LABEL,
					out);
				out.writeInt(extLength);
				writeLabel(
					PDERegistryCacheReader.EXTENSION_POINT_EXTENSIONS_LABEL,
					out);
				for (int i = 0; i < extLength; i++) {
					...
					if (extensionIndex != -1) {
						...
						writeLabel(
							PDERegistryCacheReader.EXTENSION_INDEX_LABEL,
							out);
						out.writeInt(extensionIndex);
					} else {
	...
	public void writeLibrary(LibraryModel library, DataOutputStream out) {
		try {
			...
			if ((exports = library.getExports()) != null) {
				writeLabel(
					PDERegistryCacheReader.LIBRARY_EXPORTS_LENGTH_LABEL,
					out);
	...
	public void writePluginDescriptor(
		...
		try {
			...
			if ((outString = plugin.getProviderName()) != null) {
				writeLabel(
					PDERegistryCacheReader.PLUGIN_PROVIDER_NAME_LABEL,
					out);
				out.writeUTF(outString);
			}
			...
			int extPointsSize =
				(extensionPoints == null) ? 0 : extensionPoints.length;
			if (extPointsSize != 0) {
	...
	public void writePluginFragment(
		...
		try {
			...
			if ((outString = fragment.getPluginVersion()) != null) {
				writeLabel(
					PDERegistryCacheReader.FRAGMENT_PLUGIN_VERSION_LABEL,
					out);
				out.writeUTF(outString);
			}
			if ((outByte = fragment.getMatch())
				!= PluginFragmentModel.FRAGMENT_MATCH_UNSPECIFIED) {
				writeLabel(
					PDERegistryCacheReader.FRAGMENT_PLUGIN_MATCH_LABEL,
					out);
	...
	public void writePluginPrerequisite(
		...
		try {
			...
			if ((outString = requires.getResolvedVersion()) != null) {
				writeLabel(
					PDERegistryCacheReader.REQUIRES_RESOLVED_VERSION_LABEL,
					out);
				out.writeUTF(outString);
			}
			...
			if ((outString = requires.getPlugin()) != null) {
				writeLabel(
					PDERegistryCacheReader.REQUIRES_PLUGIN_NAME_LABEL,
					out);
				out.writeUTF(outString);
			}
	...
	public void writePluginRegistry(
		PluginRegistryModel registry,
		DataOutputStream out) {
		try {
			...
			if (objectTable != null) {
				...
				if (registryIndex != -1) {
					// this plugin is already there
					writeLabel(
						PDERegistryCacheReader.REGISTRY_INDEX_LABEL,
						out);
			...
			int fragmentLength =
				(fragmentList == null) ? 0 : fragmentList.length;
