...
public class DocumentModel  implements IDocumentNode {
	...
	public DocumentModel(AbstractPluginModelBase pluginModelBase) {
		...
		fParser= XMLCore.getDefault().createXMLModelBuilder(new PluginDocumentModelFactory());
	...
	private void load(InputStream stream, boolean outOfSync, boolean initialRun) throws CoreException {
		synchronized (fPluginModelBase) {
			AbstractModelUpdateStrategy updateStrategy= reconcile(stream, outOfSync, initialRun);
	...
	public AbstractModelUpdateStrategy reconcile(InputStream stream, final boolean outOfSync, final boolean initialRun) { //called from main and reconciler thread
		synchronized (fPluginModelBase) {
			...
			boolean success;
			try {
				XMLInputSource source= new XMLInputSource(null, null, null, stream, null);
				fErrorHandler.reset();
				fParser.parse(source);
				success= fErrorHandler.getErrorCount() == 0 && fErrorHandler.getFatalErrorCount() == 0;
			...
			result= new AbstractModelUpdateStrategy(this, success) {
				public void update() {
					...
					if (pluginBase == null) {
						pluginBase= (PluginBase) fPluginModelBase.createPluginBase();
					...
					if (!initialRun) {
						fPluginModelBase.fireModelChanged(new ModelChangedEvent(IModelChangedEvent.WORLD_CHANGED, new Object[] { pluginBase }, null));
						if (fPluginModelBase instanceof IEditable && fPluginModelBase.isEditable())
	...
	static void load(PluginBase tmpPluginBase, IDocumentNode docNode) {
		...
		if (docNode != null && docNode.getChildren() != null) {
			...
			for (int i= 0; i < children.length; i++) {
				String name= children[i] instanceof PluginDocumentNode ? ((PluginDocumentNode)children[i]).getDOMNode().getNodeName().toLowerCase() : null;
				...
				for (int i= 0; i < children.length; i++) {
					if (children[i] instanceof PluginDocumentNode) {  
						...
						if (name.equals("extension")) {
							...
							iLibrary= processRuntime(tmpPluginBase, currentDocNode, iLibrary);
						} else if (name.equals("requires")) {
							iImport= processRequires(tmpPluginBase, currentDocNode, iImport);
						}
					}
				}
			}
