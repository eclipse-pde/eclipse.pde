...
public class DecoratorTemplate extends PDETemplateSection { 
	...
	private void createOptions() {
		...
		addOption(DECORATOR_ICON_PLACEMENT,
			PDETemplateMessages.DecoratorTemplate_placement,
			choices,
			choices[0][0],
			0);	
		
		projectOption = (BooleanOption) addOption(DECORATOR_BLN_PROJECT,
				PDETemplateMessages.DecoratorTemplate_decorateProject, true, 0);
		
		readOnlyOption = (BooleanOption) addOption(DECORATOR_BLN_READONLY,
				PDETemplateMessages.DecoratorTemplate_decorateReadOnly, false, 0);
		
		packageOption = addOption(
				KEY_PACKAGE_NAME,
				PDETemplateMessages.DecoratorTemplate_packageName,
				(String) null,
				0);
	...
	protected void updateModel(IProgressMonitor monitor) throws CoreException {
		...
		if(!readOnlyOption.isSelected()){
			...
			decoratorElement.setAttribute(
					"label", PDETemplateMessages.DecoratorTemplate_resourceLabel); //$NON-NLS-1$		
			...
			decoratorElement.setAttribute(
					"label", PDETemplateMessages.DecoratorTemplate_readOnlyLabel); //$NON-NLS-1$		
			decoratorElement.setAttribute(
