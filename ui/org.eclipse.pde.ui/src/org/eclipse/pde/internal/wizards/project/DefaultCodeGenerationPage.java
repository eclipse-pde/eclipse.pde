...
public class DefaultCodeGenerationPage extends WizardPage {
	...
	private void createProject(IProject project, IProgressMonitor monitor)
		...
		if (project.exists() == false) {
			CoreUtility.createProject(project, projectProvider.getLocationPath(), monitor);
	...
	private IFile generatePluginFile(
		...
		if (!fragment)
			...
			if (!last) {
				last = data.match == 0;
				appendAttribute(writer, indent, "plugin-version", data.pluginVersion, last);
				if (!last)
					appendAttribute(
						writer,
						indent,
						"match",
						IMatchRules.RULE_NAME_TABLE[data.match],
						true);
			}
		}
		writer.println();
