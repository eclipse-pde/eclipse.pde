...
*/
	...
	equivButton = factory.createButton(container, PDEPlugin.getResourceString(KEY_EQUIVALENT), SWT.RADIO);
	gd = new GridData(GridData.FILL_HORIZONTAL);
	gd.horizontalSpan = 2;
	gd.horizontalIndent = 15;
	...
	compatibleButton =
		factory.createButton(container, PDEPlugin.getResourceString(KEY_COMPATIBLE), SWT.RADIO);
	gd = new GridData(GridData.FILL_HORIZONTAL);
	gd.horizontalSpan = 2;
	gd.horizontalIndent = 15;
	...
	greaterButton =
		factory.createButton(container, PDEPlugin.getResourceString(KEY_GREATER), SWT.RADIO);
	gd = new GridData(GridData.FILL_HORIZONTAL);
	gd.horizontalSpan = 2;
	gd.horizontalIndent = 15;
...
private void buttonChanged(Button radio) {
	if (currentImport==null || blockChanges) return;
	forceDirty();
}
