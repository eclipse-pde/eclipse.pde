...
public class EditIUContainerPage extends WizardPage implements IEditBundleContainerPage {
	...
	public ITargetLocation getBundleContainer() {
		...
		if (versionSpecifications != null && !versionSpecifications.isEmpty()) {
			...
			return service.newIULocation(ids, versions, repositories, flags);
		}
		return service.newIULocation(selectedIUs, repositories, flags);
	}
		...
		fAvailableIUGroup = new AvailableIUGroup(profileUI, parent, parent.getFont(), fQueryContext, null,
			...
			protected StructuredViewer createViewer(Composite parent) {
				...
				if (fEditContainer != null) {
					...
					column.getColumn().setWidth(150); // TODO: fit to size?!
			...
			private String sanitizeVersionSpecification(String spec) {
				spec = spec.strip();
			...
			private String getVersionSpecification(Object e) {
				String spec = e instanceof AvailableIUElement iu ? versionSpecifications.getOrDefault(iu.getIU(), "") //$NON-NLS-1$
						: ""; //$NON-NLS-1$
