 ...
 * parent items the same way as {@link CachedContainerCheckedTreeViewer}
 * <p>
 * Note: If duplicate items are added to the tree the cache will treat them
 * as a single entry.
 * </p>
 */
public class CachedContainerCheckedTreeViewer extends ContainerCheckedTreeViewer {
	 ...
	 */
	protected CachedContainerCheckedTreeViewer(Tree tree) {
		super(tree);
	...
	public void setCheckedElements(Object[] elements) {
		...
		for (int i = 0; i < elements.length; i++) {
			if (!getGrayed(elements[i]) && (contentProvider == null || contentProvider.getChildren(elements[i]).length == 0)) {
