...
public class AttributePropertySource
	...
	public Object doClone() {
		...
		String value =
			PDEPlugin.getFormattedMessage(KEY_COPY_OF, att.getName());
		...
		if (descriptors == null) {
			...
			PropertyDescriptor cdesc =
				createComboBoxPropertyDescriptor(
					P_USE,
					PDEPlugin.getResourceString(KEY_USE),
					ISchemaAttribute.useTable);
			...
			cdesc =
				createComboBoxPropertyDescriptor(
					P_KIND,
					PDEPlugin.getResourceString(KEY_KIND),
					ISchemaAttribute.kindTable);
			if (cdesc instanceof ComboBoxPropertyDescriptor)
				((ComboBoxPropertyDescriptor) cdesc).setLabelProvider(
			...
			descriptors.addElement(cdesc);
			cdesc =
				new TypeRestrictionDescriptor(
					P_RESTRICTION,
					PDEPlugin.getResourceString(KEY_RESTRICTION),
					!isEditable());
			descriptors.addElement(cdesc);
			cdesc =
				createTextPropertyDescriptor(
					P_VALUE,
					PDEPlugin.getResourceString(KEY_VALUE));
			cdesc.setValidator(new ValueValidator());
			descriptors.addElement(cdesc);
			PropertyDescriptor desc =
				createTextPropertyDescriptor(
					P_BASED_ON,
					PDEPlugin.getResourceString(KEY_BASED_ON));
	...
	public Object getPropertyValue(Object name) {
		...
		if (isEditable()) {
			...
			if (name.equals(P_TYPE))
				return new Integer(
					getIndexOf(att.getType().getName(), typeTable));
		} else {
			if (name.equals(P_USE))
	...
	public void setPropertyValue(Object name, Object value) {
		...
		if (value instanceof Integer) {
			...
			if (name.equals(P_USE))
				...
				att.setType(
					new SchemaSimpleType(att.getSchema(), typeTable[index]));
