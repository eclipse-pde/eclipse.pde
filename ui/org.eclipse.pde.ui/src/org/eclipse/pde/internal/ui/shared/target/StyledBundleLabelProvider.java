	...
	private Image getInternalImage(Object element) {
		if (element instanceof TargetBundle bundle) {
			...
			if (bundle.getStatus().getSeverity() == IStatus.ERROR
				...
				// tree
				return provider.get(PDEPluginImages.DESC_FEATURE_OBJ, flag);
			} else if (bundle.isFragment()) {
				return provider.get(PDEPluginImages.DESC_FRAGMENT_OBJ, flag);
			} else if (bundle.isSourceBundle()) {
				return provider.get(PDEPluginImages.DESC_PLUGIN_MF_OBJ, flag);
			} else {
				return provider.get(PDEPluginImages.DESC_PLUGIN_OBJ, flag);
			}
		}
		if (element instanceof BundleInfo) {
			return PDEPlugin.getDefault().getLabelProvider().get(PDEPluginImages.DESC_PLUGIN_OBJ);
		}
		if (element instanceof NameVersionDescriptor descriptor) {
