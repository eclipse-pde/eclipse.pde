...
public class LauncherSection extends PDESection {
	...
	private class MacBundleUrlTypesComposite {
		...
		private static class ContentProvider implements IStructuredContentProvider {
			...
			public Object[] getElements(Object inputElement) {
				if (inputElement instanceof IProduct) {
					return ((IProduct) inputElement).getLauncherInfo().getMacBundleUrlTypes();
				}
				return new Object[0];
			}
		...
		public MacBundleUrlTypesComposite(Composite comp, FormToolkit toolkit) {
			...
			fTablePart = new MacBundleUrlTypesPartAdapter(new String[] { PDEUIMessages.LauncherSection_macBundleUrlTypes_Add,
			...
			fBundleUrlTypesTable.getTable().addKeyListener(new KeyAdapter() {
				@Override
			...
			final Table table = fBundleUrlTypesTable.getTable();
			final TableColumn schemeColumn = new TableColumn(table, SWT.LEFT);
			...
			fBundleUrlTypesTable.setContentProvider(new ContentProvider());
		...
		private void handleAdd() {
			BundleUrlTypeDialog dialog = new BundleUrlTypeDialog(PDEPlugin.getActiveWorkbenchShell(), null,
					getExistingBundleUrlTypes());
		...
		private void handleEdit() {
			...
			if (!ssel.isEmpty() && ssel.getFirstElement() instanceof IMacBundleUrlType) {
				IMacBundleUrlType propertyToEdit = (IMacBundleUrlType) ssel.getFirstElement();
				Set<IMacBundleUrlType> existing = getExistingBundleUrlTypes();
				existing.remove(propertyToEdit);
				BundleUrlTypeDialog dialog = new BundleUrlTypeDialog(PDEPlugin.getActiveWorkbenchShell(),
						propertyToEdit, existing);
				if (dialog.open() == Window.OK) {
					IMacBundleUrlType result = dialog.getResult();
					if (result != null) {
						fBundleUrlTypesTable.refresh();
		...
		private Set<IMacBundleUrlType> getExistingBundleUrlTypes() {
			Set<IMacBundleUrlType> result = new HashSet<>();
			IMacBundleUrlType[] bundleUrlTypes = getProduct().getLauncherInfo().getMacBundleUrlTypes();
			Collections.addAll(result, bundleUrlTypes);
			return result;
		}
		...
		private void handleRemove() {
			...
			if (!ssel.isEmpty()) {
				...
				IMacBundleUrlType[] bundleUrlTypes = new IMacBundleUrlType[objects.length];
				System.arraycopy(objects, 0, bundleUrlTypes, 0, objects.length);
		...
		private class BundleUrlTypeDialog extends StatusDialog {
			...
			protected void okPressed() {
				if (fEdit != null) {
					getProduct().getLauncherInfo().removeMacBundleUrlTypes(new IMacBundleUrlType[] { fEdit });
				}
				...
				getProduct().getLauncherInfo().addMacBundleUrlTypes(new IMacBundleUrlType[] { fEdit });
				super.okPressed();
			}
