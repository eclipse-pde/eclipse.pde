 ...
 * template section implementation. Options have values and visual
 * presence that allows users to change them. When a section
 * is configured with a number of options, they become 
 * available to the code generator and can take part in 
 * conditional code emitting.
 * <p>
...
public abstract class BaseOptionTemplateSection
	 ...
	 * @param name the unique name of the option (can be used as a variable
	 * in conditional code emitting and variable substitution)
	 * @param label presentable name of the option
	...
	/**
	 * Adds a choice option with a provided name, label, list of choices
	 * and the initial value (choice).
	 * @param name the unique name of the option (can be used as a variable
	 * in conditional code emitting and variable substitution)
	 * @param label presentable name of the option
	 * @param choices an array of choices that the user will have when
	 * setting the value of the option. Each array position should accept
	 * an array of String objects of size 2, the first being the unique name
	 * and the second the presentable label of the choice.
	...
	/**
	 * Initializes the option with a given unique name with the
	 * provided value. The value will be set only if the option
	...
	/**
	 * Returns a string value of the option with a given name. The
	 * option with that name must exist and must be registered as
	 * a string option to begin with.
	...
	/**
	 * Returns a boolean value of the option with a given name. The
	 * option with that name must exist and must be registered as
	 * a boolean option to begin with.
	 * @param name the unique name of the option
	...
	/**
	 * Enables the option with a given name. The exact effect of
	 * the method depends on the option type, but the end-result should
	 * always be the same - users should not be able to modify
	 * values of disabled options. This method has no effect
	...
	/**
	 * Returns the value of the option with a given name. The actual
	 * type of the returned object depends on the option type.
	 ...
	 * by the method parameters. Some options may depend on the user
	 * selection in the common wizard pages before template page
	 * has been shown (for example, plug-in ID, plug-in name etc.).
	 * This method allows options to initialize in respect to these
	 * values. 
	 ...
	 * This method allows options to initialize in respect to these
	 * values.
	 * <p>This method is called after the plug-in has already been
	 * created or as part of new extension creation (inside the
	...
	/**
	 * Subclasses must implement this method in order to validate
	...
	/**
	 * Modifies the superclass implementation by adding the 
	 * initialization step before commencing execution. This
